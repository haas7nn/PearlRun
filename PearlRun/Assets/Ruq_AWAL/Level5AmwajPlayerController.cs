...
public class Level5AmwajPlayerController : MonoBehaviour
{
    [Header("Movement")]
    ...
    void Start()
    {
        ...
        if (rb == null)
        {
            Debug.LogError("Level5AmwajPlayerController needs a Rigidbody on the Player.");
            enabled = false;
            return;
        }
    ...
    void Update()
    {
        if (isDead || (Level5AmwajRunnerGameManager.instance != null && Level5AmwajRunnerGameManager.instance.isGameOver))
        {
            StopRunningSound();
            return;
        }
